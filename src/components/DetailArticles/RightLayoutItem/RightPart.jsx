...
const RightPart = ({ loading, id, title, body, user, createdAt }) => {
  if (loading) {
    return (
      <div className="mb-4">
        <SkeletonArticle />
  ...
  return (
    <div className='col-span-3 items-start justify-start flex flex-col w-full pt-10'>
      ...
      <div className="w-full items-start justify-start pl-10 pt-3">
        <p className='font-light text-gray-500 text-xs'>published <span className='font-medium'>{postedAt(createdAt)}</span></p>
        <h1 className='text-accent-2 font-bold text-3xl pt-6 leading-10 text-justify'>{title}</h1>
